...
func TestReceiver(t *testing.T) {
	...
	receiver := &pubsubReceiver{
		logger:    zap.New(core),
		obsrecv:   obsreport.NewReceiver(obsreport.ReceiverSettings{ReceiverID: config.NewID(typeStr), Transport: reportTransport}),
		userAgent: "test-user-agent",
	...
	srv.Publish("projects/my-project/topics/otlp", testdata.CreateTraceExport(), map[string]string{
		...
		"ce-datacontenttype": "application/x-protobuf",
	})
	assert.Eventually(t, func() bool {
		return traceSink.SpansCount() > 0
	...
	assert.Eventually(t, func() bool {
		return metricSink.MetricsCount() > 0
	...
	assert.Eventually(t, func() bool {
		return logSink.LogRecordsCount() > 0
	}, time.Second, 10*time.Millisecond)
